...
public interface UpbitTicker {
     ...
     * @return quotesCurrencyCode 를 호가통화로 사용하는 종목들의 통계량을 저장하는 Map 을 반환합니다.
     * <br><br>
     * 이 Map 은 마켓 코드를 키로, {@link TickerStatistics} 를 값으로 가집니다.
     * <br><br>
     * 마켓 코드는 {호가통화코드}-{기준통화코드} 형식의 문자열이며, 호가통화코드는 반드시 quotesCurrencyCode 와 동일함이 보장됩니다.
     * <br><br>
     * 여기서 통계량은 주요 시점의 가격이나, 최대값, 또는 일정 기간동안의 누적량 등, 종목의 거래 현황을 한 눈에 보여주는 값을 말합니다.
     * 이 값들은 그렇게 자주 변하지 않기 때문에 일정 기간마다 주기적으로 업데이트 됩니다.
     */
    Map<String, TickerStatistics> getStatisticsMapWhichQuoteCurrencyCodeIs(String quoteCurrencyCode);
    ...
    /**
     * @param quoteCurrencyCode 호가통화코드 (ex. KRW, BTC, USDT)
     * @return quotesCurrencyCode 를 호가통화로 사용하는 종목들의  실시간 시세 정보를 저장하는 Map 을 반환합니다.
     * <br><br>
     * 이 Map 은 마켓 코드를 키로, {@link TickerQuotes}를 값으로 가집니다.
     * <br><br>
     * 마켓 코드는 {호가통화코드}-{기준통화코드} 형식의 문자열이며, 호가통화코드는 반드시 quotesCurrencyCode 와 동일함이 보장됩니다.
     * <br><br>
     ...
     */
    Map<String, TickerQuotes> getQuotesMapWhichQuoteCurrencyCodeIs(String quoteCurrencyCode);
